...
Nous vivons dans une société de consommation où les technologies évoluent sans cesse. La présence de systèmes embarqués dans le monde qui nous entoure ne choque plus personne. Chaque système est définit selon la ou les applications auxquelles il doit répondre. Certains systèmes ont pour objectif d'assurer la confidentialité et l'intégrité de données. C'est par exemple le cas des \textit{smart cards} (cartes à puces), utilisées quotidiennement comme moyen de paiements (carte bancaire) ou comme moyen d'identification personnelle (carte d'identité). Ces systèmes sont appelés des \textit{devices cryptographiques} car ils stockent et utilisent des algorithmes de chiffrement. 
Lorsqu'un nouvel algorithme de chiffrement se présente, certaines personnes tentent constamment d'y déceler des failles. Ceci, dans le but d'améliorer sa robustesse ou bien, dans des cas plus préoccupants, afin d'accéder à son contenu sensible. Devant cet engouement à casser les algorithmes, le \textit{NIST} (\textit{National Institute of Standards and Technology}) décida de développer un nouveau standard d'algorithme de chiffrement difficilement envisageable de casser : l'algorithme \textit{AES} (\textit{Advanced Encryption Standard}). Cet algorithme est aujourd'hui mondialement connu et implémenté dans la majeure partie des systèmes embarqués comportant des applications en sécurité. 

Cependant, au début des années 2000, un nouveau type d'attaque ciblant les algorithmes de chiffrement a vu le jour : les attaques par canaux auxiliaires (\textit{side channel attacks}). Lorsqu'elles sont maîtrisées, ces attaques peuvent devenir extrêmement puissantes et casser un algorithme (notamment l'AES) en un rien de temps. Un exemple est l'attaque par \textit{l'analyse de la consommation de puissance}. Cette attaque (il en existe plusieurs) est liée à la consommation de puissance du device cryptographique lorsqu'il chiffre des données. Ceci découle du fait que la puissance consommée par le circuit intégré reflète directement ses activités internes (instructions exécutées et données manipulées).
